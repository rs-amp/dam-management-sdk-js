...
interface DamFixtureParameters {
  selectors?: string[];
...
export class DAMFixtures {
  static install(adapter: MockAdapter): void {
    ...
    /*
    this.installSpecific(adapter, fixturesBaseDir + '/assets', {
      id: 'c3361f53-6a0c-4e14-825d-fcff771f42a3',
      selectors: ['select', 'external'],
    });
    */
  ...
  private static installSpecific(
    adapter: MockAdapter,
    baseDir: string,
      ...
      if (dirName[i] === '{') {
        for (let j = i + 1; j < dirName.length; j++) {
          if (dirName[j] === '}') {
            ...
            if (params[paramName] != null) {
              dirName =
                dirName.substring(0, i) +
                params[paramName] +
                dirName.substring(j + 1);
              break;
            } else {
              return; // Skip this file.
            }
